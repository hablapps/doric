...
class DeserializeSparkTypeSpec extends DoricTestElements {
  ...
  describe("Complex types"){
    it("should match a combination of Spark types"){
      ...
      deserializeSparkType[Array[Array[User]]](Array(Array(User("",0), User("",0)), Array()))
      deserializeSparkType[Array[List[User]]](Array(List(User("",0), User("",0)), List()))
      deserializeSparkType[Map[Int, Option[List[User]]]](Map(0->None, 1->Some(List(User("",0)))))
      deserializeSparkType[(List[Int], User, Map[Int, Option[User]])]((List(0,0), User("",0), Map(0->None, 1->Some(User("",0)))))
    }
  }
