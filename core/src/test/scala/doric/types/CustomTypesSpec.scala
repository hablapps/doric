...
class CustomTypesSpec extends DoricTestElements {
  ...
  describe("Custom types within complex types"){
    ...
    it("should deserialize/serialize properly in Arrays") {
      ...
      serializeSparkType[IndexedSeq[User]](IndexedSeq(User("n1",10), User("n2", 10)))

    }

    it("should match Spark Map types") {

      serializeSparkType[Map[Int, User]](Map(0 -> User("n1",1), 1 -> User("n2",2)))
      serializeSparkType[Map[User, Int]](Map(User("n1",1) -> 0, User("n2",2) -> 1))
    }
    ...
    ignore("should match case classes") {
      ...
      SparkType[(User, String)].dataType should ===(
        StructType(Array(StructField("_1",StringType,true), StructField("_2",StringType,true)))
      )
