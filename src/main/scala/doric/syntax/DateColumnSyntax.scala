...
trait DateColumnSyntax {
  implicit class DateColumnLikeSyntax[T: DateType](
    ...
    /**
      * @return an Integer DoricColumn with the day number of the date.
      */
    def dayOfMonth: IntegerColumn = column.elem.map(functions.dayofmonth).toDC

    /**
      * Adds to the date the number of months
      * @param nMonths the number of months to add, can be negative to substract.
      * @return DoricColumn of the same type as the original with the month changed.
      */
